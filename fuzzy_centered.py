...
def create_centered_parking_system():
    ...
    dist_lateral.add_term("muito_acima", "trapmf", [-80, -80, -40, -20])
    dist_lateral.add_term("acima", "trimf", [-30, -15, -5])
    dist_lateral.add_term("centrado", "trimf", [-8, 0, 8])     
    dist_lateral.add_term("abaixo", "trimf", [5, 15, 30])
    dist_lateral.add_term("muito_abaixo", "trapmf", [20, 40, 80, 80])
    ...
    profundidade.add_term("entrada", "trapmf", [0, 0, 10, 30])
    profundidade.add_term("inicio", "trimf", [20, 40, 58])
    profundidade.add_term("centro", "trimf", [55, 70, 85])
    profundidade.add_term("fundo", "trimf", [80, 105, 130])
    profundidade.add_term("final", "trapmf", [125, 140, 150, 150])
    ...
    velocidade.add_term("parado", "trapmf", [0, 0, 1, 3])
    velocidade.add_term("muito_lento", "trimf", [2, 5, 10])
    velocidade.add_term("lento", "trimf", [8, 15, 25])
    velocidade.add_term("medio", "trimf", [20, 35, 55])
    velocidade.add_term("rapido", "trapmf", [50, 70, 100, 100])
...
if __name__ == "__main__":
    ...
    test_cases = [
        (610, 290, 10, "Logo após entrar"),
        (650, 290, 50, "Meio da vaga"),
        (675, 290, 75, "Centro ideal"),
        (700, 290, 100, "Próximo do centro"),
        (730, 290, 130, "Próximo do final")
    ]
    
    for x, y, prof, desc in test_cases:
        inputs = {
            'distancia_frontal': 750 - x,
            'distancia_lateral': 0,
            'angulo_veiculo': 0,
            'profundidade_vaga': prof
        }
